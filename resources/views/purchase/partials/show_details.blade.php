...
<div class="modal-body">
  ...
  <div class="row invoice-info">
    <div class="col-sm-4 invoice-col">
      ...
      <address>
        ...
        @endif
        @foreach(\Arr::get(json_decode($purchase->contact->business->custom_labels, true), 'contact') as $key => $label)
        @if(!empty($label))
        <br>{{ $label }}: {{ $purchase->contact->{\Str::replaceLast('_', '', $key)} }}
        @endif
        @endforeach
      </address>
  ...
  @if($signs = \Arr::get($purchase->contact->business->common_settings, 'purchase_print_signs'))
  @php
  $signs = explode(',', $signs);
  @endphp
  <div class="row">
    <div class="col">
      <table class="table table-no-split text-center">
        <tr>
  ...
  {{-- Barcode --}}
  @if(\Arr::get($purchase->contact->business->common_settings, 'enable_purchase_print_barcode'))
...
<style>
  .table-no-split.print-friendly tr td,
  .table-no-split.print-friendly tr th {
